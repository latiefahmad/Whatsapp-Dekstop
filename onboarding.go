		...
		(function() {
			...
			function showOnboarding(force) {
				...
				intro.innerHTML = '<div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">' +
					...
					'<h1 id="wa-onboarding-title" style="font-size:18px;line-height:1.3;font-weight:600;letter-spacing:-.15px;margin:0;">WhatsApp Desk siap digunakan</h1>' +
					'</div>' +
					'<p style="font-size:13px;line-height:1.55;color:' + muted + ';margin:0;max-width:58ch;">Masuk atau pindai kode QR seperti biasa. Kontrol desktop tersedia saat Anda membutuhkannya.</p>';
				panel.appendChild(intro);

				var rows = document.createElement('div');
				rows.style.cssText = 'border-top:1px solid ' + border + ';border-bottom:1px solid ' + border + ';margin-bottom:20px;';
				var items = [
					['Samarkan isi percakapan', 'Cmd/Ctrl + Shift + P'],
					['Pertahankan jendela di depan', 'Cmd/Ctrl + Shift + T'],
					['Buka pengaturan', 'Cmd/Ctrl + ,']
				];
				...
				skip.textContent = 'Lewati panduan';
				...
				continueButton.textContent = 'Lanjut ke WhatsApp';
